...
const App: React.FC = () => {
  ...
  const handleConnectLinkedIn = async () => {
    try {
      ...
      addLog("AUTH_ERROR: Failed to initialize LinkedIn handshake.", "error");
    }
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#050505] text-white">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-[#0a0a0a] border-r border-[#1a1a1a] flex flex-col transition-transform duration-300
        ...
        <div className="mt-auto p-6 space-y-4">
           ...
           <div className="p-2">
              ...
              <div className="h-1 bg-[#222] rounded-full overflow-hidden">
                 <div className="h-full bg-green-500 transition-all duration-1000 shadow-[0_0_8px_rgba(34,197,94,0.4)]" style={{ width: `${(currentRevenue / REVENUE_GOAL) * 100}%` }} />
              </div>
           </div>
           
           <button onClick={() => { localStorage.removeItem('ra_auth'); window.location.reload(); }} className="w-full text-[9px] text-gray-700 uppercase font-black hover:text-white transition-colors">Terminate Session</button>
        </div>
      </aside>

      {/* Workspace */}
      <div className="flex-1 flex flex-col relative overflow-hidden bg-[#050505]">
        <header className="h-20 border-b border-[#1a1a1a] flex items-center justify-between px-10 bg-[#0a0a0a]/50 backdrop-blur-xl z-20">
           <div className="flex items-center gap-4">
              <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden text-gray-400">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
              </button>
              <h2 className="text-xs font-black text-gray-400 uppercase tracking-[0.4em] italic truncate">
      ...
      {isMobileMenuOpen && (
        <div onClick={() => setIsMobileMenuOpen(false)} className="fixed inset-0 bg-black/80 backdrop-blur-md z-[45] lg:hidden" />
      )}
    </div>
  );
};
...
const NavItem = ({ active, onClick, icon, label }: any) => (
  <button
    ...
    className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${
      active ? 'bg-white text-black font-black shadow-xl scale-[1.02]' : 'text-gray-500 hover:text-white hover:bg-[#111]'
    }`}
  >
...
const StatBox = ({ label, val, color = "text-white" }: any) => (
  <div className="flex flex-col items-end">
    <span className="text-[9px] font-black text-gray-700 uppercase tracking-widest mb-0.5">{label}</span>
    <span className={`text-xl font-black mono ${color} tracking-tighter`}>{val}</span>
  </div>
);
